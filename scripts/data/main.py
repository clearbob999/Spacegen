...
if __name__ == '__main__':
    ...
    parser.add_argument('--match_brick_npy', type=str, default='data/FGFR4_matched_bbs_emb_256.npy')
    parser.add_argument('--match_brick_txt', type=str, default='data/FGFR4_matched_bbs.txt')
    parser.add_argument('--match_rules', type=str, default='data/FGFR4_rxn_filter.txt')
    parser.add_argument('--receptor_file', type=str, default='data/cdk9_8k5r.pdbqt')
    parser.add_argument('--rule_json_path', type=str, default='data/FGFR4_data_for_reaction_filtered.json.gz')


    parser.add_argument('--max_episodes', type=int, default=200)
    parser.add_argument('--max_action', type=int, default=2, help='max step to generate molecule')
    ...
    docking_config = get_docking_config_for_vina(args.receptor_file)
    ...
    # with open('data/rxn_set_filter.txt', 'r') as f:
    #     rxns = [l for l in f.readlines()]
    #     args.t_dim = len(rxns)

    with open(args.match_brick_txt, 'r') as f:
        starting_smi = [l.strip() for l in f.readlines()]
        print(len(starting_smi))
    with open(args.match_rules, 'r') as f:
        rxns = [l for l in f.readlines()]
        args.t_dim = len(rxns)
