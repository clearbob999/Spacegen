...
class SynthesisEnv(gym.Env):
    ...
    def init(self, args, start_smis, predictor, max_action=5):    
        ...
        self.mol_mw = 500
        # with open(self.args.match_brick_pkl, 'rb') as file:
        #     self.bb_emb = pickle.load(file)
        self.bb_emb = np.load(self.args.match_brick_npy)
        self.building_blocks = [line.strip() for line in open(self.args.match_brick_txt, 'r')]
        self.bb_dict = {self.building_blocks[i]: i for i in range(len(self.building_blocks))}
        
        rxn_set = ReactionSet()
        rxn_set.load(args.rule_json_path)
        ...
        self.rxn_mask = get_reaction_mask(self.smi, self.rxns)  # 只有0，1 能和不能反应
