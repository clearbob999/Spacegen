...
def Fragment_Space_Search(smiles_input,BB_data,fingerprint,Endpoint_threshold
                          ,Scaffold_threshold,break_list,chemprop_checkboxgroup,fragment1,fragment2,break_choice):
    ...
    print(search_args.cut_num)
    search_args.Endpoint_threshold,search_args.Scaffold_threshold = Endpoint_threshold,Scaffold_threshold
    search_args.break_list = [break_list[break_choice-1]]
    print(search_args.break_list)
    search_args.cal_property = chemprop_checkboxgroup
    if search_args.cut_num == 2:
        if fragment2 == 'Fragment1':
            search_args.fragment = 0
        elif fragment2 == 'Scaffold':
            search_args.fragment = 1
        else:
            search_args.fragment = 2
    else:
        if fragment2 == 'Fragment1':
            search_args.fragment = 0
        else:
    ...
    replace_result, search_result, dfs = us.Replace_dummys(search_args.break_list,search_args.fragment)
    product_df = us.get_result(search_args.break_list, dfs)
    print(product_df)
...
class Universe_search:
    ...
    def Replace_dummys(self,initial_results,fix_flag):
        '''
        ...
        if self.args.cut_num == 1:
            ...
            for i, result in enumerate(initial_results):
                ...
                terminal_products.drop_duplicates(
                    subset=['Reactant1', 'Reactant1_ID', 'Reactant2', 'Reactant2_ID', 'Product'], keep='first',
                    inplace=True)
                # terminal_products.drop_duplicates(subset=['Product'], keep='first',inplace=True)
    ...
    def brick_to_rule(self, result_df):
        ...
        for index, reaction in enumerate(reactions):
            smirks_series = rules_df.loc[rules_df['Name_reactions'] == reaction, 'Smirks'].values[0]
...
if __name__ == '__main__':
    # break_list = Bricap_break('Cc1cc(NC(=O)c2ccc(NS(=O)(=O)CCO)cc2N2CCC3(CC2)CC3)nc(N2CCC(F)(F)CC2)n1'
    #                           ,5,2)

    break_list = Bricap_break('Cc1cc(NC(=O)c2ccc(NS(=O)(=O)CCO)cc2N2CCC3(CC2)CC3)nc(N2CCC(F)(F)CC2)n1'
                              , 5, 1)

    # Fragment_Space_Search('Cc1cc(NC(=O)c2ccc(NS(=O)(=O)CCO)cc2N2CCC3(CC2)CC3)nc(N2CCC(F)(F)CC2)n1'
    #                       ,"Enamine BB Catalog", "Morgan", 0.4, 0.4
    #              , break_list, ['MW', 'HAC','slogP','HBA','HBD','TPSA','RotBonds','QED','SAscore']
    #                       , 'scaffold', None,1)

    Fragment_Space_Search('Cc1cc(NC(=O)c2ccc(NS(=O)(=O)CCO)cc2N2CCC3(CC2)CC3)nc(N2CCC(F)(F)CC2)n1'
                          , "Enamine BB Catalog", "Morgan", 0.33, 0.33
                          , break_list, ['MW', 'HAC', 'slogP', 'HBA', 'HBD', 'TPSA', 'RotBonds', 'QED', 'SAscore']
                          , None, 'Fragment1', 1)
