...
class ReplayBuffer:
    def __init__(self, max_size, state_dim, t_dim, action_dim, batch_size):
        ...
        self.state_memory = np.zeros((max_size, state_dim))  #256
        self.t_memory = np.zeros((max_size, t_dim))   #112
        self.t_mask_memory = np.zeros((max_size, t_dim))    #112
        self.action_memory = np.zeros((max_size, action_dim))   #256
        self.reward_memory = np.zeros((max_size, ))   #无
        self.next_state_memory = np.zeros((max_size, state_dim))   #256
        self.done_memory = np.zeros((max_size, ), dtype=np.bool_)    #无
